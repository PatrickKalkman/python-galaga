...
class PathPointSelector():
    ...
    def find_control_points_of_path_point(self, control_point_handler: ControlPointHandler):
        ...
        if control_point_handler.control_point_index == 0:
            ...
            related_control_points.append(ControlPointHandler(
            ...
            if control_point_handler.quartet_index == 0:
                ...
                related_control_points.append(ControlPointHandler(
                    control_point_handler.quartet_index - 1, 2))

        elif control_point_handler.control_point_index == 3:

            related_control_points.append(ControlPointHandler(
                control_point_handler.quartet_index, 2))

            if control_point_handler.quartet_index == 0 and number_of_quartets > 1:

                related_control_points.append(ControlPointHandler(
                    control_point_handler.quartet_index + 1, 1))
    ...
    def get_control_point_pairs(self):
        ...
        line_list.append(ControlPointHandler(1, 0))
        line_list.append(ControlPointHandler(last_quartet_index, 2))

        if self.get_number_of_quartets > 1:
            for _, index in enumerate(self.control_point_quartet_collection):
                line_list.append(ControlPointHandler(index, 2))
                line_list.append(ControlPointHandler(index + 1, 1))
